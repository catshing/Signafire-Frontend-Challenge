...
const Toolbar = (props) => (
    <div>
        <h2 id="toolbar__starred">
            Starred: {props.showTrashed ? (
            props.trashedMessages.filter(msg => msg.meta.isStarred === true).length
            ) : (
            props.messages.filter(msg => msg.meta.isStarred === true).length
        )}
        </h2>
        <div className="toolbar__toggleTrashedMessages">
            <button type="button" className="toolbar__trashButton" onClick={props.toggleTrashedMessages}>
                <span>
                {
